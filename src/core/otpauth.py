...
#         return token
    ...
    def hotp(self, counter=4):
        based_secret = base64.b32decode(self.secret, True)
        struct_secret = struct.pack(">Q", counter)
        hmac_secret = hmac.new(based_secret, struct_secret, hashlib.sha1).digest()
        ob = hmac_secret[19]
        if python_version == 2:
            ob = ord(ob)
        ord_secret = ob & 15
        token_secret = (struct.unpack(">I", hmac_secret[ord_secret:ord_secret+4])[0] & 0x7fffffff) % 1000000
        return token_secret
    ...
    def valid_totp(self, code, period=30):
        ...
        :param period: A period that a TOTP code is valid in seconds
        """
        return valid_code(code) and int(code) in self.totp(period)
